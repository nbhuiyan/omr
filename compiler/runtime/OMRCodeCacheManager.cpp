...
OMR::CodeCacheManager::destroy()
   ...
   if (_elfRelocatableGenerator)
      {
        _elfRelocatableGenerator->emitELF((const char*)_objectFileName,
   ...
   if (_elfGenerator)
      ...
      if (numCharsWritten > 0 && numCharsWritten < maxElfFilenameSize)
         {
         _elfGenerator->emitELF((const char*) &elfFilename, 
...
OMR::CodeCacheManager::registerCompiledMethod(const char *sig, uint8_t *startPC, uint32_t codeSize)
   ...
   if (_elfRelocatableGenerator){
      CodeCacheSymbol *newRelocSymbol = static_cast<CodeCacheSymbol *> (self()->getMemory(sizeof(CodeCacheSymbol)));
      memcpy(newRelocSymbol, newSymbol, sizeof(CodeCacheSymbol));
...
OMR::CodeCacheManager::registerStaticRelocation(const TR::StaticRelocation &relocation)
   ...
   if (_elfRelocatableGenerator)
      ...
      CodeCacheRelocationInfo *newRelocation = static_cast<CodeCacheRelocationInfo *> (self()->getMemory(sizeof(CodeCacheRelocationInfo)));
      newRelocation->_location = relocation.location();
      newRelocation->_type = relocationType;
      newRelocation->_symbol = _relocatableSymbolContainer->_numSymbols - 1; //symbol index along the linked list
...
OMR::CodeCacheSymbolContainer * OMR::CodeCacheManager::_symbolContainer = NULL;
OMR::CodeCacheSymbolContainer * OMR::CodeCacheManager::_relocatableSymbolContainer = NULL;
//OMR::CodeCacheRelocationInfoContainer * OMR::CodeCacheManager::_relocations = NULL;

void
OMR::CodeCacheManager::initializeRelocatableELFGenerator()
   {
   _objectFileName = TR::Options::getCmdLineOptions()->getObjectFileName();
   OMR::CodeCacheSymbolContainer * symbolContainer = static_cast<CodeCacheSymbolContainer *>(self()->getMemory(sizeof(CodeCacheSymbolContainer)));
   _relocatableSymbolContainer = symbolContainer;
   _relocatableSymbolContainer->_head = NULL;
   _relocatableSymbolContainer->_tail = NULL;
   _relocatableSymbolContainer->_numSymbols = 0; //does not include UNDEF symbol
   _relocatableSymbolContainer->_totalSymbolNameLength = 1; //precount for the UNDEF symbol

   
   OMR::CodeCacheRelocationInfoContainer * _relocations = static_cast<CodeCacheRelocationInfoContainer *>(self()->getMemory(sizeof(CodeCacheRelocationInfoContainer)));
   _relocations->_head = NULL;
   _relocations->_tail = NULL;
   _relocations->_numRelocations = 0;

   _elfRelocatableGenerator =
      new (_rawAllocator) OMR::ELFGenerator(
         _rawAllocator,
         _codeCacheRepositorySegment->segmentBase(),
...
void
OMR::CodeCacheManager::initializeELFGenerator()
   {
   CodeCacheSymbolContainer * symbolContainer = static_cast<CodeCacheSymbolContainer *>(self()->getMemory(sizeof(CodeCacheSymbolContainer)));
   _symbolContainer = symbolContainer;
   _symbolContainer->_head = NULL;
   _symbolContainer->_tail = NULL;
   _symbolContainer->_numSymbols = 0; //does not include UNDEF symbol
   _symbolContainer->_totalSymbolNameLength = 1; //precount for the UNDEF symbol
   _elfGenerator =
      new (_rawAllocator) OMR::ELFGenerator(
         _rawAllocator,
         _codeCacheRepositorySegment->segmentBase(),
