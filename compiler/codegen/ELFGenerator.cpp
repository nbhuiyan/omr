...
TR::ELFGenerator::writeELFSymbolsToFile(::FILE *fp){
    ...
    ELFSymbolNames[0] = 0; //first bit needs to be 0, for the UNDEF symbol
    char * names = ELFSymbolNames + 1; //the rest of the array will contain symbol names
    ...
    while(sym){
        ...
        elfSym->st_value = sym->_start ? (ELFAddress)(sym->_start - _codeStart) : 0;
...
TR::ELFGenerator::writeRelaEntriesToFile(::FILE *fp){
    if(_numRelocations > 0){
        ...
        while (reloc){
            ...
            elfRelas->r_info = ELF64_R_INFO(reloc->_symbol+1, reloc->_type);
...
TR::ELFExecutableGenerator::buildSectionHeaders(void){
    ...
    uint32_t trailerStartOffset = sizeof(ELFEHeader) + sizeof(ELFProgramHeader) +
                                  _codeSize;
    uint32_t symbolsStartOffset = trailerStartOffset + (sizeof(ELFSectionHeader) * 
                                  /* #shdr */ 5) + shStrTabNameLength;
    uint32_t symbolNamesStartOffset = symbolsStartOffset + ((_numSymbols + /* UNDEF */ 1)
                                      * sizeof(ELFSymbol)) + shStrTabNameLength;
